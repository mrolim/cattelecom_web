...
(function($){
	$.fn.highlight = function(element_params){
		...
		return this.each(function(){
			...
			code = code.replace(/\bscript2\b/gi, 'script'); //fix script
			code = code.replace(/\bhtml2\b/gi, 'html'); //fix script
			code = code.replace(/\bhead2\b/gi, 'head'); //fix script
			code = code.replace(/\bbody2\b/gi, 'body'); //fix script
			code = code.replace(/\b2DOCTYPE\b/gi, 'DOCTYPE'); //fix script
	...
	$.highlightCode = {
		...
		hightlight_html: function(code) {
			...
			code = code
				...
				//replace open tags
				.replace(/(&lt;)(\w{0,15})(\s+|&gt;|>)/gi,'<span class="html_tagop">$1</span><span class="tag">$2</span><span class="html_tagop">$3</span>')
				//replace close tags
				.replace(/(&lt;)\/(\w{0,15})(&gt;|>)/gi,'<span class="html_tagop">$1/</span><span class="tag">$2</span><span class="html_tagop">$3</span>')
				//replace doctype
